...
export type MarcoTempo = { data: string; titulo: string; descricao: string };

// O campo "linha_do_tempo" é digitado como texto simples, uma linha por
// marco, no formato "data | título | descrição" -- assim evitamos um
// formulário complexo com botões de adicionar/remover campo.
export function parseLinhaDoTempo(texto: string | null): MarcoTempo[] {
  if (!texto) return [];
  return texto
    .split("\n")
    ...
    .filter(Boolean)
    .map((linha) => {
      const [data = "", titulo = "", descricao = ""] = linha
        .split("|")
        .map((p) => p.trim());
...
export type Promessa = { icone: string; titulo: string; descricao: string };

// Ícones que giram automaticamente para cada promessa, na ordem em que
// forem digitadas -- assim a pessoa não precisa escolher ícone manualmente.
const ICONES_PROMESSA = ["∞", "🏡", "✨", "✈", "🌱", "🌳", "☾", "☀"];

// Cada promessa é digitada como "título | descrição", uma por linha.
export function parsePromessas(texto: string | null): Promessa[] {
  if (!texto) return [];
  return texto
    .split("\n")
    .map((l) => l.trim())
    ...
    .map((linha, i) => {
      ...
      return {
        icone: ICONES_PROMESSA[i % ICONES_PROMESSA.length],
        titulo: titulo || linha,
        descricao,
      };
    });

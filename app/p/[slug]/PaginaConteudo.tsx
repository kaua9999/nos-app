...
export default function PaginaConteudo({ pagina }: { pagina: Pagina }) {
  ...
  return (
    <main className={`relative flex-1 overflow-hidden ${estilo.fundo}`}>
          ...
          <div className="max-w-xl mx-auto">
            ...
            <div className="space-y-4">
              {promessas.map((p, i) => (
                ...
                >
                  <span className="shrink-0 w-11 h-11 rounded-xl bg-gold/15 border border-gold/30 flex items-center justify-center text-xl text-gold-soft">
                    {p.icone}
                  </span>
                  <div>
      ...
      {/* CARTA + MÚSICA */}
      <section className="relative z-10 px-6 py-20 pb-16 text-center flex flex-col items-center gap-4">
      ...
      <footer className="relative z-10 pb-12 text-center text-xs text-[color:var(--muted)] font-mono">
        feito com amor ❤️ para {pagina.nome_casal}
      </footer>
    </main>
  );
}
